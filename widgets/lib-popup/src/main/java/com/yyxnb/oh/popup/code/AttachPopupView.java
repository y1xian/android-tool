...
import com.yyxnb.oh.popup.animator.PopupAnimation;
import com.yyxnb.oh.popup.animator.PopupAnimator;
import com.yyxnb.oh.popup.animator.ScrollScaleAnimator;
import com.yyxnb.oh.popup.enums.PopupPosition;
import com.yyxnb.oh.popup.util.PopupUtils;
import com.yyxnb.oh.popup.widget.PartShadowContainer;
...
public abstract class AttachPopupView extends BasePopupView {
    ...
    protected void applyBg(){
        if (!popupInfo.hasShadowBg) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                attachPopupContainer.setElevation(PopupUtils.dp2px(getContext(), 20));
            } else {
                //优先使用implView的背景
                if (getPopupImplView().getBackground() == null) {
                    defaultOffsetX -= bgDrawableMargin;
                    defaultOffsetY -= bgDrawableMargin;
                    attachPopupContainer.setBackgroundResource(R.drawable._popup_shadow);
                } else {
                    Drawable.ConstantState constantState = getPopupImplView().getBackground().getConstantState();
                    if(constantState!=null){
                        Drawable newDrawable = constantState.newDrawable();
    ...
    protected void doAttach() {
        overflow = PopupUtils.dp2px(getContext(), overflow);
        ...
        if (popupInfo.touchPoint != null) {
            ...
            int maxHeight = (int) (isShowUpToTarget() ? (popupInfo.touchPoint.y - PopupUtils.getStatusBarHeight() - overflow)
                                : (PopupUtils.getWindowHeight(getContext()) - popupInfo.touchPoint.y - overflow));
            int maxWidth = (int) (isShowLeft ? (PopupUtils.getWindowWidth(getContext())-popupInfo.touchPoint.x-overflow) : (popupInfo.touchPoint.x-overflow));
            ...
            getPopupContentView().post(new Runnable() {
                ...
                public void run() {
                    if(isRTL){
                        translationX = isShowLeft ?  -(PopupUtils.getWindowWidth(getContext())-popupInfo.touchPoint.x-getPopupContentView().getMeasuredWidth()- defaultOffsetX)
                                : -(PopupUtils.getWindowWidth(getContext())-popupInfo.touchPoint.x + defaultOffsetX);
                    }else {
            ...
            ViewGroup.LayoutParams params = getPopupContentView().getLayoutParams();
            int maxHeight = isShowUpToTarget() ? (rect.top - PopupUtils.getStatusBarHeight() - overflow)
                    : (PopupUtils.getWindowHeight(getContext()) - rect.bottom - overflow);
            ...
            getPopupContentView().post(new Runnable() {
                ...
                public void run() {
                    if(isRTL) {
                        translationX = isShowLeft ?  -(PopupUtils.getWindowWidth(getContext())-rect.left-getPopupContentView().getMeasuredWidth() - defaultOffsetX)
