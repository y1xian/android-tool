package com.yyxnb.oh.popup.code;
...
import com.yyxnb.oh.popup.PopupManager;
import com.yyxnb.oh.popup.animator.BlurAnimator;
import com.yyxnb.oh.popup.animator.EmptyAnimator;
import com.yyxnb.oh.popup.animator.PopupAnimator;
import com.yyxnb.oh.popup.animator.ScaleAlphaAnimator;
import com.yyxnb.oh.popup.animator.ScrollScaleAnimator;
import com.yyxnb.oh.popup.animator.ShadowBgAnimator;
import com.yyxnb.oh.popup.animator.TranslateAlphaAnimator;
import com.yyxnb.oh.popup.animator.TranslateAnimator;
import com.yyxnb.oh.popup.enums.PopupStatus;
import com.yyxnb.oh.popup.impl.FullScreenPopupView;
import com.yyxnb.oh.popup.impl.PartShadowPopupView;
import com.yyxnb.oh.popup.util.KeyboardUtils;
import com.yyxnb.oh.popup.util.PopupUtils;
import com.yyxnb.oh.popup.util.navbar.NavigationBarObserver;
import com.yyxnb.oh.popup.util.navbar.OnNavigationBarListener;

import java.util.ArrayList;
import java.util.List;
import java.util.Stack;

import static com.yyxnb.oh.popup.animator.PopupAnimation.NoAnimation;
...
public abstract class BasePopupView extends FrameLayout implements OnNavigationBarListener, LifecycleObserver {
    ...
    protected void init() {
        ...
        if (!isCreated) {
            ...
            if (popupInfo.popupCallback != null) {
                popupInfo.popupCallback.onCreated(this);
            }
        }
    ...
    private Runnable initTask = new Runnable() {
        ...
        public void run() {
            ...
            if (popupInfo.popupCallback != null) {
                popupInfo.popupCallback.beforeShow(BasePopupView.this);
            }
    ...
    private void collectAnimator() {
        if (popupContentAnimator == null) {
            ...
            if (popupInfo.hasBlurBg) {
                ...
                blurAnimator.decorBitmap = PopupUtils.view2Bitmap((PopupUtils.context2Activity(this)).getWindow().getDecorView());
    ...
    private Runnable doAfterShowTask = new Runnable() {
        ...
        public void run() {
            ...
            if (popupInfo != null && popupInfo.popupCallback != null) {
                popupInfo.popupCallback.onShow(BasePopupView.this);
            }
            //再次检测移动距离
            if (dialog == null) {
                return;
            }
            if (PopupUtils.getDecorViewInvisibleHeight(dialog.getWindow()) > 0 && !hasMoveUp) {
                PopupUtils.moveUpToKeyboard(PopupUtils.getDecorViewInvisibleHeight(dialog.getWindow()), BasePopupView.this);
            }
        }
    };
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_UP) {
                if (popupInfo.isDismissOnBackPressed &&
                        (popupInfo.popupCallback == null || !popupInfo.popupCallback.onBackPressed(BasePopupView.this))) {
    ...
    public int getAnimationDuration() {
        return popupInfo.popupAnimation == NoAnimation ? 10 : PopupManager.getAnimationDuration() + 10;
    }
    ...
    public void dismiss() {
        ...
        clearFocus();
        if (popupInfo.popupCallback != null) {
            popupInfo.popupCallback.beforeDismiss(this);
        }
        beforeDismiss();
        doDismissAnimation();
        doAfterDismiss();
    ...
    public void smartDismiss() {
        handler.post(new Runnable() {
            @Override
            public void run() {
                delayDismiss(PopupManager.getAnimationDuration() + 50);
            }
        });
    }
    ...
    protected void doAfterDismiss() {
        ...
        if (popupInfo.autoOpenSoftInput && !(this instanceof PartShadowPopupView)) {
    ...
    private Runnable doAfterDismissTask = new Runnable() {
        ...
        public void run() {
            ...
            onDismiss();
            if (popupInfo != null && popupInfo.popupCallback != null) {
                popupInfo.popupCallback.onDismiss(BasePopupView.this);
            }
