package com.yyxnb.oh.music.service;
...
import com.yyxnb.oh.music.MusicPlayerManager;
import com.yyxnb.oh.music.R;
import com.yyxnb.oh.music.interfaces.IMusic;
import com.yyxnb.oh.music.interfaces.IMusicPlayer;
import com.yyxnb.oh.music.interfaces.MusicConstants;
import com.yyxnb.oh.music.interfaces.MusicPlayerEventListener;
import com.yyxnb.oh.music.interfaces.MusicPlayerInfoListener;
import com.yyxnb.oh.music.utils.Logger;
import com.yyxnb.oh.music.utils.MusicAudioFocusManager;
import com.yyxnb.oh.music.utils.MusicImageCache;
import com.yyxnb.oh.music.utils.MusicRomUtil;
import com.yyxnb.oh.music.utils.MusicStatus;
import com.yyxnb.oh.music.utils.MusicUtils;
 ...
 */
public class MusicPlayerService extends Service implements IMusicPlayer, MediaPlayer.OnPreparedListener,
    ...
    //待播放音频队列池子
    private static List<IMusic> mAudios = new ArrayList<>();
    //当前播放播放器正在处理的对象位置
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case TIME_MSG:
                    ...
                    if (getPlayerState() == MusicConstants.MUSIC_PLAYER_PLAYING
                            || getPlayerState() == MusicConstants.MUSIC_PLAYER_PAUSE) {
                        //UI类型处理事件
...
//                        Logger.d("---", "更新进度 ：" + mMediaPlayer.getDuration() + " , " + mMediaPlayer.getCurrentPosition() + 500);
    ...
    public void onCreate() {
        ...
        intentFilter.addAction(MusicConstants.MUSIC_INTENT_ACTION_CLICK_WORD);
     ...
     * 设置数据源
     *
     * @param musicList 待播放的数据集，对象需要继承 IMusic
     * @param index     指定要播放的位置 0-data.size()
     */
    @Override
    public <T extends IMusic> void setMusic(List<T> musicList, int index) {
        if (null != musicList) {
            mAudios.clear();
            ...
            if (null != mOnPlayerEventListeners) {
                for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                    onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "设置数据源，播放准备中");
                }
            }
        }
    }
     ...
     */
    @Override
    public <T extends IMusic> void startPlayMusic(List<T> musicList, int index) {
        if (null != musicList) {
            mAudios.clear();
            mAudios.addAll(musicList);
    ...
    public void startPlayMusic(int index) {
        ...
        if (null != mAudios && mAudios.size() > index) {
            this.mCurrentPlayIndex = index;
            IMusic baseMusicInfo = (IMusic) mAudios.get(index);
    ...
    public <T extends IMusic> void addPlayMusicToTop(T audioInfo) {
        ...
        IMusic playerMusic = getCurrentPlayerMusic();
        if (null != playerMusic && playerMusic.audioId().equals(audioInfo.audioId())) {
            return;
        }
        if (mAudios.size() > 0) {
            int pisition = -1;
            for (int i = 0; i < mAudios.size(); i++) {
                IMusic musicInfo = (IMusic) mAudios.get(i);
                if (audioInfo.audioId().equals(musicInfo.audioId())) {
                    pisition = i;
                    break;
                }
            }
    ...
    public void pause() {
        try {
            ...
            if (null != mOnPlayerEventListeners) {
                for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                    onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "暂停播放");
                }
            }
    ...
    private void passivePause() {
        try {
            ...
            if (null != mOnPlayerEventListeners) {
                for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                    onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "暂停播放，失去焦点");
                }
            }
            MusicPlayerManager.getInstance().observerUpdata(new MusicStatus(MusicStatus.PLAYER_STATUS_PAUSE));
            //最后更新通知栏
    ...
    public void play() {
        ...
        try {
            ...
            if (null != mMediaPlayer) {
                ...
                if (null != mOnPlayerEventListeners) {
                    for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                        onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "播放中，恢复播放");
                    }
                }
                //最后更新通知栏
                showNotification();
...
//            ((IMusic) mAudios.get(mCurrentPlayIndex)).setAudioPath(sourcePath);
            startPlayMusic(mCurrentPlayIndex);
        }
    }
    ...
    public void continuePlay(String sourcePath, int index) {
        ...
        if (null != mAudios && mAudios.size() > index) {
//            ((IMusic) mAudios.get(index)).setAudioPath(sourcePath);
            startPlayMusic(index);
        }
    }
...
//                    break;
    ...
    public String getCurrentPlayerId() {
        ...
        if (null != mAudios && mAudios.size() > mCurrentPlayIndex) {
            return ((IMusic) mAudios.get(mCurrentPlayIndex)).audioId();
        }
        return "";
    }
    ...
    public <T extends IMusic> T getCurrentPlayerMusic() {
        ...
        if (null != mAudios && mAudios.size() > mCurrentPlayIndex) {
            return (T) mAudios.get(mCurrentPlayIndex);
        }
        return null;
    }
    ...
    public void onCheckedCurrentPlayTask() {
        if (null != mMediaPlayer && null != mAudios && mAudios.size() > 0) {
            if (null != mOnPlayerEventListeners) {
                IMusic musicInfo = mAudios.get(mCurrentPlayIndex);
                try {
                    for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
    ...
    public void addOnPlayerEventListener(MusicPlayerEventListener listener) {
        if (null != mOnPlayerEventListeners) {
            ...
            removeDuplicate(mOnPlayerEventListeners);
//            Logger.e("---", "addOnPlayerEventListener  : " + mOnPlayerEventListeners.size());
        }
    }

    // 去掉重复数据
    private void removeDuplicate(List<MusicPlayerEventListener> list) {
        List<MusicPlayerEventListener> result = new ArrayList<>(list.size());
        for (MusicPlayerEventListener str : list) {
    ...
    public MusicPlayerManager setPlayInfoListener(MusicPlayerInfoListener listener) {
        MusicPlayerService.sMusicPlayerInfoListener = listener;
        return null;
    }
    ...
    public void removePlayInfoListener() {
        MusicPlayerService.sMusicPlayerInfoListener = null;
    }
    ...
    public void onStop() {
        ...
        try {
            ...
            if (null != mOnPlayerEventListeners) {
                for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                    onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "停止播放");
                }
            }
        }
    }
...
//        MusicWindowManager.getInstance().createMiniJukeBoxToWindown(getApplicationContext());
     ...
     */
    @SuppressWarnings("unchecked")
    private synchronized void startPlay(IMusic musicInfo) {
        onReset();
        if (null != musicInfo && !TextUtils.isEmpty(musicInfo.audioPath())) {
            startTimer();
            if (null == mAudioFocusManager) {
                mAudioFocusManager = new MusicAudioFocusManager(MusicPlayerService.this.getApplicationContext());
            }
...
//                    SqlLiteCacheManager.getInstance().insertCollectAudio(musicInfo);
                ...
                try {
                    ...
                    String path = getPlayPath(musicInfo.audioPath());
                    Logger.e(TAG, "startPlay-->: ID:" + musicInfo.audioId() + ",TITLE:"
                            + musicInfo.audioTitle() + ",PATH:" + path);
            ...
            if (null != mOnPlayerEventListeners && mOnPlayerEventListeners.size() > 0) {
                for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                    onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "播放失败，无数据源或地址错误");
    ...
    public void onCompletion(MediaPlayer mediaPlayer) {
        ...
        if (null != mOnPlayerEventListeners) {
            for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "播放结束");
            }
        }
    ...
    public void onSeekComplete(MediaPlayer mediaPlayer) {
        ...
        if (mMusicAlarmModel == MusicConstants.MUSIC_ALARM_MODEL_CURRENT) {
            ...
            TIMER_DURTION = (duration / 1000);
        }
        MusicPlayerService.this.mMusicPlayerState = MusicConstants.MUSIC_PLAYER_PLAYING;
        if (null != mOnPlayerEventListeners) {
            for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "播放中，进度调整");
            }
        }
    }
    ...
    public boolean onInfo(MediaPlayer mediaPlayer, int event, int extra) {
        ...
        if (null != mOnPlayerEventListeners) {
            for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, message);
                onPlayerEventListener.onInfo(event, extra);
            }
        }
        return false;
    }
    ...
    private class HeadsetBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(AudioManager.ACTION_AUDIO_BECOMING_NOISY)) {
                ...
                String audioId = intent.getStringExtra(MusicConstants.MUSIC_KEY_MEDIA_ID);
                if (!audioId.isEmpty()) {
                ...
                IMusic currentPlayerMusic = getCurrentPlayerMusic();
                if (null != currentPlayerMusic) {
//                    boolean existToCollectByID = SqlLiteCacheManager.getInstance().isExistToCollectByID(currentPlayerMusic.audioId());
//                    if (existToCollectByID) {
//                        SqlLiteCacheManager.getInstance().deteleCollectByID(currentPlayerMusic.audioId());
//                    } else {
...
//                    }
                ...
                //词
            } else if (action.equals(MusicConstants.MUSIC_INTENT_ACTION_CLICK_WORD)) {
    ...
    private void showNotification(int state) {
        if (mNotificationEnable) {
            ...
            if (isOpen) {
                IMusic audioInfo = getCurrentPlayerMusic();
                if (null != audioInfo) {
                    //先准备好歌曲封面Bitmap
                    if (audioInfo.audioPath().startsWith("http:") || audioInfo.audioPath().startsWith("https:")) {
                        RequestOptions options = new RequestOptions();
                        options.placeholder(R.drawable.ic_music_default_cover)
                        ...
                        Glide.with(getApplicationContext())
                                ...
                                .apply(options)
                                .into(new SimpleTarget<Drawable>(120, 120) {
                                    @Override
                                    public void onResourceReady(@NonNull Drawable resource, @Nullable Transition<? super Drawable> transition) {
                                        if (null == resource) {
                                            resource = getResources().getDrawable(R.drawable.ic_music_default_cover);
                                        }
                                        BitmapDrawable bd = (BitmapDrawable) resource;
                                        Notification notification = buildNotifyInstance(getCurrentPlayerMusic(), bd.getBitmap());
                                        showNotification(notification, NOTIFICATION_ID, mForegroundEnable);
                                    }
                                });
                    } else {
                        //File
                        Bitmap bitmap;
                        bitmap = MusicImageCache.getInstance().getBitmap(audioInfo.audioPath());
                        //缓存为空，获取音频文件自身封面
                        if (null == bitmap) {
                            bitmap = MusicImageCache.getInstance().createBitmap(audioInfo.audioPath());
                        }
                        //封面为空，使用默认
                        if (null == bitmap) {
     ...
     * @return 通知对象
     */
    private <T extends IMusic> Notification buildNotifyInstance(T audioInfo, Bitmap cover) {
        if (null == audioInfo) {
            return null;
        }
        //8.0及以上系统需创建通知通道
        ...
        rootIntent.putExtra(MusicConstants.MUSIC_KEY_MEDIA_ID, audioInfo.audioId());
     ...
     * @return RemoteView
     */
    private <T extends IMusic> RemoteViews getDefaultCoustomRemoteView(T audioInfo, Bitmap cover) {
        final RemoteViews defaultremoteviews = new RemoteViews(getPackageName(), R.layout.music_notify_default_controller);
        ...
        defaultremoteviews.setTextViewText(R.id.music_notice_def_title, audioInfo.audioTitle());
        defaultremoteviews.setTextViewText(R.id.music_notice_def_subtitle, audioInfo.audioSubTitle());
        //上一首
        ...
        //词
        defaultremoteviews.setOnClickPendingIntent(R.id.music_notice_def_btn_word,
                getClickPending(MusicConstants.MUSIC_INTENT_ACTION_CLICK_WORD));
     ...
     * @return RemoteView
     */
    private <T extends IMusic> RemoteViews getBigCoustomRemoteView(T audioInfo, Bitmap cover) {
        final RemoteViews bigRemoteViews = new RemoteViews(getPackageName(), R.layout.music_notify_big_controller);
        ...
        bigRemoteViews.setImageViewResource(R.id.music_notice_def_btn_collect, getCollectIcon(mState));
        bigRemoteViews.setTextViewText(R.id.music_notice_def_title, audioInfo.audioTitle());
        bigRemoteViews.setTextViewText(R.id.music_notice_def_subtitle, audioInfo.audioSubTitle());
        //上一首
        ...
        //词
        bigRemoteViews.setOnClickPendingIntent(R.id.music_notice_def_btn_word,
                getClickPending(MusicConstants.MUSIC_INTENT_ACTION_CLICK_WORD));
    ...
    private PendingIntent getClickPending(String action) {
        final Intent lastIntent = new Intent(action);
        final PendingIntent lastPendIntent = PendingIntent.getBroadcast(MusicPlayerService.this,
     ...
     * @return 收藏状态对应的RES
     */
    private int getCollectIcon(int mState) {
        return mState == 1 ? R.drawable.ic_music_collect_pre : R.drawable.ic_music_collect_noimal;
    }
