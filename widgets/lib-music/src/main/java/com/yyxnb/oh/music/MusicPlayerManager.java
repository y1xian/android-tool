...
import com.yyxnb.oh.music.interfaces.MusicInitializeCallBack;
...
import com.yyxnb.oh.music.service.MusicPlayerBinder;
import com.yyxnb.oh.music.service.MusicPlayerService;
import com.yyxnb.oh.music.utils.MusicPlayerConfig;
import com.yyxnb.oh.music.utils.MusicSubjectObservable;
 ...
 */
public final class MusicPlayerManager implements IMusicPlayer {
...
//        MusicWindowManager.getInstance().onDestroy();
    ...
    public MusicPlayerManager setPlayerActivityName(String className) {
        MusicPlayerManager.mActivityPlayerClassName = className;
        if (null != mBinder && mBinder.pingBinder()) {
    ...
    public MusicPlayerManager setLockActivityName(String activityClassName) {
        MusicPlayerManager.mActivityLockClassName = activityClassName;
        if (null != mBinder && mBinder.pingBinder()) {
    ...
    public <T extends IMusic> void setMusic(List<T> musicList, int index) {
        if (null != mBinder && mBinder.pingBinder()) {
            mBinder.setMusic(musicList, index);
        }
    }
    ...
    public void updateNotification(int state) {
        if (null != mBinder && mBinder.pingBinder()) {
            mBinder.updateNotification(state);
        }
    }
