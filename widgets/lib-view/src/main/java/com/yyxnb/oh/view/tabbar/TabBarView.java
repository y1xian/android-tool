package com.yyxnb.oh.view.tabbar;
...
import com.yyxnb.oh.core.DpUtils;
import com.yyxnb.oh.core.interfaces.OnSelectListener;
import com.yyxnb.oh.view.R;
...
public class TabBarView extends LinearLayout {
    ...
    private void laodAttrs(Context context, AttributeSet attrs) {
        ...
        height = typedArray.getLayoutDimension(R.styleable.TabBarView_android_layout_height, -2);
        iconPadding = typedArray.getDimensionPixelOffset(R.styleable.TabBarView_tb_iconPadding, iconPadding);
        textSize = typedArray.getDimensionPixelOffset(R.styleable.TabBarView_tb_textSize, textSize);
        tabPaddingVertical = typedArray.getDimensionPixelOffset(R.styleable.TabBarView_tb_paddingVertical, tabPaddingVertical);
        focusColor = typedArray.getColor(R.styleable.TabBarView_tb_focusColor, focusColor);
        normalColor = typedArray.getColor(R.styleable.TabBarView_tb_normalColor, normalColor);
        tabClickBackground = typedArray.getInt(R.styleable.TabBarView_tb_clickBackground, TabClickBackgroundValue.RIPPLE.ordinal());
        paddingNavigationBar = typedArray.getBoolean(R.styleable.TabBarView_tb_paddingNavigationBar, paddingNavigationBar);
        noDyeing = typedArray.getBoolean(R.styleable.TabBarView_tb_noDyeing, noDyeing);
        noSelect = typedArray.getBoolean(R.styleable.TabBarView_tb_noSelect, noSelect);
        splitLine = typedArray.getResourceId(R.styleable.TabBarView_tb_splitLine, splitLine);
        unreadBackground = typedArray.getResourceId(R.styleable.TabBarView_tb_unreadBackground, unreadBackground);
    ...
    public TabBarView setTab(List<Tab> datas) {
        ...
        for (int i = 0; i < tabDatas.size(); i++) {
            ...
            final View item = LayoutInflater.from(context).inflate(R.layout._tab_view_layout, null, false);
    ...
    private void refreshBackground(View item) {
        if (tabClickBackground == TabClickBackgroundValue.RIPPLE.ordinal()) {
            ...
            item.setBackgroundResource(R.drawable._tab_gray_background);
        } else {
        ...
        for (int i = 0; i < tabViews.size(); i++) {
            ...
            item.setOnClickListener(v -> {
                ...
                if (onSelectListener != null) {
                    onSelectListener.onClick(v, focusIndex, "");
                }
            });
        }
    }
     ...
     */
    public TabBarView setOnSelectListener(OnSelectListener onSelectListener) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (context != null) {
            if (paddingNavigationBar && !isInEditMode()) {
                int navHeight = DpUtils.getNavigationBarHeight(context);
