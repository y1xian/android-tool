...
import com.yyxnb.view.popup.code.DrawerPopup;
import com.yyxnb.view.popup.code.FullScreenPopup;
import com.yyxnb.view.popup.code.PartShadowPopup;
...
//                    w = Math.min(w, implView.getMeasuredWidth());
    ...
    public static void moveUpToKeyboard(int keyboardHeight, BasePopup pv) {
        if (!pv.popupInfo.isMoveUpToKeyboard) {
            return;
        }
        if (pv instanceof PositionPopup) {
            return;
        }
        //判断是否盖住输入框
        ...
        if (!(pv instanceof PartShadowPopup) && pv instanceof AttachPopup) {
            return;
        }
        //执行上移
        if (pv instanceof FullScreenPopup ||
        ...
        } else if (pv instanceof CenterPopup) {
    ...
    private static boolean isBottomPartShadow(BasePopup pv) {
        return pv instanceof PartShadowPopup && ((PartShadowPopup) pv).isShowUp;
    }

    private static boolean isTopPartShadow(BasePopup pv) {
        return pv instanceof PartShadowPopup && !((PartShadowPopup) pv).isShowUp;
    }

    public static void moveDown(BasePopup pv) {
        //暂时忽略PartShadow弹窗和AttachPopupView
        if (pv instanceof PositionPopup) {
            return;
        }
        ...
        if (pv instanceof PartShadowPopup && !isBottomPartShadow(pv)) {
