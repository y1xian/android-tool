...
public class PermissionFragment extends Fragment {
	...
	private IPermissionListener permissionCheckListener;
	...
	public PermissionFragment setPermissionCheckListener(IPermissionListener listener) {
	...
	public void onActivityCreated(@Nullable Bundle savedInstanceState) {
		...
		forceDeniedPermissionTips = "请前往设置->应用->【" + AppUtil.getAppName(mContext.get()) + "】->权限中打开相关权限，否则功能无法正常运行！";
...
//                        String denied = deniedString.substring(0, deniedString.length() - 1);
	...
	private void requestPermissionsSuccess() {
		...
		mContext.get().getSupportFragmentManager().beginTransaction().remove(this).commit();
	}
