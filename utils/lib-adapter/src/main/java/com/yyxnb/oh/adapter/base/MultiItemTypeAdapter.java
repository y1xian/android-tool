package com.yyxnb.oh.adapter.base;
...
import com.yyxnb.oh.adapter.ItemDelegateManager;
import com.yyxnb.oh.adapter.OnItemClickListener;
...
public class MultiItemTypeAdapter<T> extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    private final ItemDelegateManager<T> mItemDelegateManager = new ItemDelegateManager<>();
     ...
     * 刷新新数据
     */
    public void setDataItems(List<T> data) {
        if (this.mData == null) {
    ...
    public void addDataItem(@NonNull Collection<? extends T> newData) {
        ...
        notifyItemRangeInserted(preSize - newData.size() + getHeaderCount(), newData.size());
        compatibilityDataSizeChanged(newData.size());
    }
    ...
    public void addDataItem(@IntRange(from = 0) int position, @NonNull T data) {
        ...
        notifyItemInserted(position + getHeaderCount() - 1);
