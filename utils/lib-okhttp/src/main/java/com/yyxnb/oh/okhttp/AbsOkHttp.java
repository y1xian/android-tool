...
import com.yyxnb.oh.okhttp.interceptor.HeaderInterceptor;
import com.yyxnb.oh.okhttp.interceptor.LogInterceptor;
import com.yyxnb.oh.okhttp.interceptor.NetworkInterceptor;
import com.yyxnb.oh.okhttp.interceptor.weaknetwork.WeakNetworkInterceptor;
import com.yyxnb.oh.okhttp.utils.SSLUtils;
...
public abstract class AbsOkHttp {
     ...
     * 开启主机名校验
     */
    protected boolean isHostnameVerifier() {
        return true;
    }
     ...
     * SslSocket
     */
    protected boolean isSslSocketFactory() {
        return true;
    }
    ...
    private OkHttpClient defaultOkHttpClient() {
        ...
        if (isHostnameVerifier()) {
            ...
            builder.hostnameVerifier((hostname, sslSession) -> {
                if (TextUtils.isEmpty(hostname) || ArrayUtil.isEmpty(hostnameVerifier())) {
                    return false;
                }
                if (ArrayUtil.contains(hostnameVerifier(), hostname)) {
                    return true;
                } else {
