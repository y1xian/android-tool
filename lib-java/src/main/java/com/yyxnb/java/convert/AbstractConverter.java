...
public abstract class AbstractConverter<T> implements Converter<T>, Serializable {
	...
	public T convert(Object value, T defaultValue) {
		...
		if (null == targetType && null == defaultValue) {
			throw new NullPointerException(StrUtil.format("[type] and [defaultValue] are both null for Converter [%s], we can not know what type to convert !", this.getClass().getName()));
		}
		if (null == targetType) {
			// 目标类型不确定时使用默认值的类型
		...
		if (null == defaultValue || targetType.isInstance(defaultValue)) {
			...
			throw new IllegalArgumentException(
					StrUtil.format("Default value [%s](%s) is not the instance of [%s]", defaultValue, defaultValue.getClass(), targetType));
		}
	}
