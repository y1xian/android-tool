package com.yyxnb.java.thread;

import com.yyxnb.java.Builder;
...
public class ThreadFactoryBuilder implements Builder<ThreadFactory> {
	...
	public ThreadFactoryBuilder setPriority(int priority) {
		if (priority < Thread.MIN_PRIORITY) {
			throw new IllegalArgumentException(StrUtil.format("Thread priority (%s) must be >= %s", priority, Thread.MIN_PRIORITY));
		}
		if (priority > Thread.MAX_PRIORITY) {
			throw new IllegalArgumentException(StrUtil.format("Thread priority (%s) must be <= %s", priority, Thread.MAX_PRIORITY));
		}
