...
import com.yyxnb.java.collection.MapUtil;
import com.yyxnb.java.func.Supplier0;
...
public class Assert {
	 ...
	 */
	public static <X extends Throwable> void isTrue(boolean expression, Supplier0<? extends X> supplier) throws X {
		if (!expression) {
			throw supplier.get();
		}
	}
	 ...
	 * @throws X if expression is {@code false}
	 */
	public static <X extends Throwable> void isFalse(boolean expression, Supplier0<X> errorSupplier) throws X {
		if (expression) {
	 ...
	 */
	public static <X extends Throwable> void isNull(Object object, Supplier0<X> errorSupplier) throws X {
		if (null != object) {
			throw errorSupplier.get();
		}
	}
	 ...
	 */
	public static <T, X extends Throwable> T notNull(T object, Supplier0<X> errorSupplier) throws X {
		if (null == object) {
			throw errorSupplier.get();
		}
		return object;
	 ...
	 */
	public static <T extends CharSequence, X extends Throwable> T notEmpty(T text, Supplier0<X> errorSupplier) throws X {
	 ...
	 */
	public static <T extends CharSequence, X extends Throwable> T notBlank(T text, Supplier0<X> errorMsgSupplier) throws X {
	 ...
	 */
	public static <T extends CharSequence, X extends Throwable> T notContain(CharSequence textToSearch, T substring, Supplier0<X> errorSupplier) throws X {
	...
	public static String notContain(String textToSearch, String substring) throws IllegalArgumentException {
		return notContain(textToSearch, substring, "[Assertion failed] - this String argument must not contain the substring [%s]", substring);
	}
	 ...
	 */
	public static <T, X extends Throwable> T[] notEmpty(T[] array, Supplier0<X> errorSupplier) throws X {
	 ...
	 */
	public static <T, X extends Throwable> T[] noNullElements(T[] array, Supplier0<X> errorSupplier) throws X {
	 ...
	 */
	public static <E, T extends Iterable<E>, X extends Throwable> T notEmpty(T collection, Supplier0<X> errorSupplier) throws X {
	 ...
	 */
	public static <K, V, T extends Map<K, V>, X extends Throwable> T notEmpty(T map, Supplier0<X> errorSupplier) throws X {
	...
	public static void isAssignable(Class<?> superType, Class<?> subType) throws IllegalArgumentException {
		isAssignable(superType, subType, "%s is not assignable to %s)", subType, superType);
	}
	 ...
	 */
	public static void state(boolean expression, Supplier0<String> errorMsgSupplier) throws IllegalStateException {
		if (false == expression) {
	...
	public static int checkBetween(int value, int min, int max) {
		if (value < min || value > max) {
			throw new IllegalArgumentException(StrUtil.format("Length must be between %s and %s.", min, max));
		}
		return value;
	}
	 ...
	 */
	public static <X extends Throwable> int checkBetween(int value, int min, int max, Supplier0<? extends X> errorSupplier) throws X {
		if (value < min || value > max) {
			throw errorSupplier.get();
		}
	...
	private static String badIndexMsg(int index, int size, String desc, Object... params) {
		if (index < 0) {
			return StrUtil.format("%s (%s) must not be negative", StrUtil.format(desc, params), index);
		} else if (size < 0) {
			throw new IllegalArgumentException("negative size: " + size);
		} else { // index >= size
			return StrUtil.format("%s (%s) must be less than size (%s)", StrUtil.format(desc, params), index, size);
		}
	}
	// ----------------------------------------------------------------------- Private method end
}
