...
public final class ServiceUtils {
    ...
    public static Set getAllRunningServices() {
        ActivityManager am = (ActivityManager) AppConfig.getInstance().getApp().getSystemService(Context.ACTIVITY_SERVICE);
    ...
    public static void startService(@NonNull final Class<?> cls) {
        Intent intent = new Intent(AppConfig.getInstance().getApp(), cls);
        AppConfig.getInstance().getApp().startService(intent);
    }
    ...
    public static boolean stopService(@NonNull final Class<?> cls) {
        Intent intent = new Intent(AppConfig.getInstance().getApp(), cls);
        return AppConfig.getInstance().getApp().stopService(intent);
    }
    ...
    public static void bindService(@NonNull final Class<?> cls,
                                   ...
                                   final int flags) {
        Intent intent = new Intent(AppConfig.getInstance().getApp(), cls);
        AppConfig.getInstance().getApp().bindService(intent, conn, flags);
    }
    ...
    public static void unbindService(@NonNull final ServiceConnection conn) {
        AppConfig.getInstance().getApp().unbindService(conn);
    }
