...
import com.yyxnb.utils.ConstConfig;
...
import static com.yyxnb.utils.ConstConfig.*;
...
public class DataUtils {
     ...
     * @param byteNum 字节数
     * @param unit    <ul>
     *                <li>{@link ConstConfig.MemoryUnit#BYTE}: 字节</li>
     *                <li>{@link ConstConfig.MemoryUnit#KB}  : 千字节</li>
     *                <li>{@link ConstConfig.MemoryUnit#MB}  : 兆</li>
     *                <li>{@link ConstConfig.MemoryUnit#GB}  : GB</li>
     *                </ul>
     * @return 以unit为单位的size
     */
    public static double byte2Size(long byteNum, ConstConfig.MemoryUnit unit) {
        if (byteNum < 0) {
            return -1;
        }
        switch (unit) {
     ...
     * @return 字节数
     */
    public static long size2Byte(long size, ConstConfig.MemoryUnit unit) {
        if (size < 0) {
            return -1;
        }
        switch (unit) {
    ...
    public static String changeDistance(double length, boolean displayMeter) {
        if (length < 1000) {
            return ConstConfig.FORMAT_TWO.format(length) + (displayMeter ? "米" : "");
        } else {
            return ConstConfig.FORMAT_TWO.format(length / 1000) + (displayMeter ? "千米" : "");
        }
    }
